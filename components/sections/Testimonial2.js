                        ...
                        <div className="col-lg-6 col-md-12 pd_right_60">
                            <div className="image_box_only  type_six color_two">
                                <div className="image one">
                                    <img src={data?.testimonials_photos} alt="img" className="img-fluid" />
                            ...
                            <div className="testimonial_carousel  position-relative position_one nav_false dot_true dot_left color_three">
                                <Swiper {...swiperOptions} className="theme_carousel ">
                                    {data?.process_list?.map((listItem) => {
                                        const starArray = Array.from({ length: 5 }, (_, index) => index < listItem.stars);
                                        return (
                                        <SwiperSlide key={listItem.id} className="testimonial_box type_two color_two">
                                            <div className="auth d-flex align-items-center clearfix">
                                                ...
                                                <div className="cont">
                                                    <h4 className="title_no_a_24">{listItem?.["name_" + locale]}</h4>
                                                    <p>{listItem?.["position_" + locale]}</p>
                                                </div>
                                            </div>
                                            <div className="title_no_a_22 t_comment">
                                                <sup>“</sup>
                                                {listItem?.["commentary_" + locale]}
                                                <sub>“</sub>
                                            </div>
                                            <div className="box">
                                                <h6>{listItem.testimonial}</h6>
                                                <p>
                                                    {starArray.map((filled, index) => (
                                                        <i key={index} className={`fa fa-star ${filled ? 'fill' : 'empty'}`} />
                                                    ))}
                                                </p>
                                            </div>
                                        </SwiperSlide>
